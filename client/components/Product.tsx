    ...
    return (
        <Card style={Styles.container}>
            <Card.Content style={Styles.userContainer}>
                <Avatar.Image size={24} source={props.product.user.avatar} style={{ backgroundColor: 'transparent' }} />
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Card.Content style={Styles.contentContainer}>
                    <Text style={{ fontWeight: 'bold' }}>  <IconLibary name='like2'></IconLibary> {likes} Likes</Text>
                    <View style={Styles.likesContainer}>
                        <IconLibary name={likeIcon} onPress={() => likeClick()} size={30} />
                        <IconLibary style={{ paddingLeft: 7 }} name={DislikeIcon} onPress={() => dislikeClick()} size={30} />
                    </View>
                    <Paragraph><Text style={{ fontWeight: 'bold' }}>{props.product.user.name}</Text> {props.product.title}</Paragraph>
                </Card.Content>
                <View style={{ flex: 1 }}>
                    <Card.Actions >
                        <Button
                            ...
                            onPress={() => { Linking.openURL(props.product.url) }}
                        >
                            לחנות !
                        </Button>
                    </Card.Actions>
